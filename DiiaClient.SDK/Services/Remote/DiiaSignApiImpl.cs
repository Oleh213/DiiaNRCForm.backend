...
namespace DiiaClient.SDK.Services.Remote
{
    internal class DiiaSignApiImpl : IDiiaSignApi
    {
            ...
            string returnLink = null)
        {
            ...
            {
                byte[] hashBytes = sha256.ComputeHash(Encoding.UTF8.GetBytes(requestId));

                string base64Hash = Convert.ToBase64String(hashBytes);
                
                return new AuthDeepLink()
                {
                    DeepLink = await diiaSharingService.GetDeepLink(branchId, offerId, base64Hash, returnLink),
                    RequestId = requestId,
                    RequestIdHash = base64Hash
                };
            }
        }
